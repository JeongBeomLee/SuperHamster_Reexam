...
void RemoteProjectileManager::Initialize(uint32_t poolSize)
{
    m_poolSize = poolSize;
...
shared_ptr<GameObject> RemoteProjectileManager::SpawnProjectile(const Vec3& position, const Vec3& direction)
{
...
void RemoteProjectileManager::Update()
{
...
void RemoteProjectileManager::InitializeParticleEffects()
{
...
shared_ptr<GameObject> RemoteProjectileManager::CreateProjectileObject(const Vec3& position, const Vec3& direction)
{
...
void RemoteProjectileManager::ReturnToPool(shared_ptr<GameObject> projectile)
{
    m_projectilePool.push(projectile);
...
void RemoteProjectileManager::PlayEffects(const Vec3& position, const Vec3& direction)
{
    // MuzzleFlash 설정 및 재생
    ...
    muzzleDesc.material = GET_SINGLE(Resources)->Get<Material>(L"MuzzleFlash")->Clone();
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        shockDesc.material = GET_SINGLE(Resources)->Get<Material>(L"Shockwave")->Clone();

        shockSystem->Play(shockDesc);
    }
}

void RemoteProjectileManager::PlayCollisionEffect(const Vec3& position)
{
    ...
    collisionDesc.material = GET_SINGLE(Resources)->Get<Material>(L"CollisionEffect")->Clone();
