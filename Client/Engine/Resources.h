...
public:
	...
	shared_ptr<Mesh> LoadLineMesh();
	shared_ptr<Mesh> LoadLineMesh(const Vec3& start, const Vec3& end);

	shared_ptr<Mesh> LoadRectangleMesh();
	shared_ptr<Mesh> LoadRectangleMesh(float width, float height, float sliceX = 1, float sliceY = 1);

	shared_ptr<Mesh> LoadCubeMesh();
	shared_ptr<Mesh> LoadCubeMesh(float width, float height, float depth);

	shared_ptr<Mesh> LoadSphereMesh();
	shared_ptr<Mesh> LoadSphereMesh(int32 sliceCount, int32 stackCount, float sphereRadius = 0.5f);

	shared_ptr<Mesh> LoadTerrainMesh(int32 sizeX = 15, int32 sizeZ = 15);

	shared_ptr<Mesh> LoadCapsuleMesh(float radius = 0.5f, float height = 1.0f);
